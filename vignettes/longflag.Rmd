...
vignette: >
  %\VignetteIndexEntry{Getting started with longflag}
...
If you have not yet installed the package, you can do so with:
...
longflag() helps answer these questions by:
 ...
 - allowing you to specify a numeric threshold for what you consider meaningful;
...
You must provide the names (as character strings) of three variables:
...
We can quickly summarize how many timepoints each subject has:
...
You can adjust the threshold to be more or less stringent. For instance, to see the distribution of absolute changes and choose a quantile-based cutoff:
...
This method is useful when you care about the average trajectory rather than just the net change between the first and last measurements.
...
This method is helpful when you want to pinpoint exactly when large changes occur. For example, to list only flagged intervals:
...
Which method you use depends on your scientific question:

 - Use "first_last" when you are interested in the overall change from baseline (or first observation) to the last available measurement.

 - Use "mean_change" when you want a summary of the average rate of change over time.

 - Use "all_timepoints" when you need to know at which specific intervals substantial changes occurred.

In practice, you might try multiple methods to understand how sensitive their conclusions are to the definition of change.


## Further work

This vignette has focused on the basic use of longflag(). In your own analyses, you might:
...
We encourage you to explore the function help (?longflag) and adapt the examples shown here to your own longitudinal data.
